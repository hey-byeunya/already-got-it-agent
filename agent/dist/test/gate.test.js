...
import { createGate, approvalTarget } from '../src/gate.js';
...
async function tempRuns(fn) {
    ...
    try {
        ...
        if (prev === undefined)
            delete process.env.OPS_RUNS_DIR;
        else
            process.env.OPS_RUNS_DIR = prev;
...
test('승인 게이트 (canUseTool)', async (t) => {
    await t.test('쓰기 도구는 기본 거절이다', async () => {
        await tempRuns(async () => {
            ...
            const res = await gate(ISSUE_TOOL, { repo: 'o/r', title: 't', body: 'b' }, {});
    ...
    await t.test('승인하면 approval_token 을 주입한다 — 모델은 토큰을 받지 않는다', async () => {
        await tempRuns(async (dir) => {
            ...
            const res = await gate(ISSUE_TOOL, input, {});
            assert.equal(res?.behavior, 'allow');
            const updated = res.updatedInput;
            assert.ok(typeof updated.approval_token === 'string' && updated.approval_token.startsWith('apr_'), '승인 결과에 토큰이 실려야 한다');
            assert.equal(input.hasOwnProperty('approval_token'), false, '원래 입력(모델이 보낸 것)에는 토큰이 없어야 한다');
    ...
    await t.test('승인 대상을 입력에서 찾을 수 없으면 거절한다', async () => {
        await tempRuns(async () => {
            const gate = mkGate(allowAll);
            const res = await gate(ISSUE_TOOL, { title: 't' }, {}); // repo 없음
            assert.equal(res?.behavior, 'deny');
            assert.match(res.message, /승인 대상/);
        });
    });
    await t.test('AskUserQuestion 은 답을 updatedInput.answers 로 되돌린다', async () => {
        await tempRuns(async () => {
            ...
            const res = await gate('AskUserQuestion', { questions }, {});
            assert.equal(res?.behavior, 'allow');
            const updated = res.updatedInput;
    ...
    await t.test('경계 — 쓰기 도구는 allowedTools 에 절대 들어가지 않는다', () => {
        for (const w of WRITE_TOOLS) {
            assert.equal(READ_TOOLS.includes(w), false, `${w} 가 allowedTools 에 들어가면 canUseTool 이 건너뛰어져 게이트가 무력화된다`);
        }
    });
});
