...
const REAL_WARNING = 'canUseTool will not be invoked for: mcp__ops__get_system_health, mcp__ops__get_user_metrics,'
...
function emitShadowWarning(message) {
...
test('셰도잉 감시', async (t) => {
    ...
    await t.test('쓰기 도구가 가려지면 실패로 처리한다', async () => {
        ...
        g.arm();
        try {
            emitShadowWarning(`canUseTool will not be invoked for: ${READ_TOOLS[0]}, ${WRITE_TOOLS[0]}.`);
            await new Promise((r) => setImmediate(r));
