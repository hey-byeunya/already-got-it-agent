...
test('종료 조건', async (t) => {
    ...
    await t.test('환경변수로 상한을 낮출 수 있다 (종료 조건 캡처용)', () => {
        ...
        try {
            ...
            delete process.env.OPS_MAX_TOOL_CALLS;
        }
    });
});
test('사용량 집계', async (t) => {
    const assistant = (id, usage, parent) => ({ type: 'assistant', parent_tool_use_id: parent ?? null, message: { id, usage } });
...
//# sourceMappingURL=limits-usage.test.js.map