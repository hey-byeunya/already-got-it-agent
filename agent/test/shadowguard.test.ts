...
test('셰도잉 감시', async (t) => {
  await t.test('해당 경고가 뜨면 실패로 처리한다', async () => {
    ...
    g.arm();
    try {
      const w = new Error('canUseTool shadowed by allowedTools entry') as Error & { code?: string };
      w.name = 'Warning';
  ...
  await t.test('disarm 뒤에는 더 듣지 않는다', async () => {
    const g = new ShadowGuard();
    g.arm(); g.disarm();
    const w = new Error('x') as Error & { code?: string };
    w.name = 'Warning'; w.code = 'CLAUDE_SDK_CAN_USE_TOOL_SHADOWED';
    process.emit('warning', w);
    await new Promise((r) => setImmediate(r));
    assert.equal(g.shadowed, null);
  });
