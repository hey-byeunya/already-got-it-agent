...
**구현 완료 (CLI 로 확인 가능).** `cd agent && npm run check` — 검사 97개.
...
- [x] `PreToolUse` 훅 (게이트 ③) — `agent/src/hook.ts`, 검사 13개 (설정 무관 불변식 2종)
...
- [x] 자동 채점 스크립트 `agent/src/eval/score.ts` — **검사 51개**, 과탐지 9건 수정 이력 포함
- [x] 평가 실행기 `agent/src/eval/run.ts` (`--rescore` 로 무료 재채점)
- [x] 시행 기록에 **원고 전문 저장** — 재채점과 수동 채점의 근거
- [x] 기준 실행 — 픽스처 4개 × 3회 = 12시행, 전부 완주 (`eval-results/E0/`)
- [x] 수동 항목 판정 — f3 원인 단정 회피, f4 결측·0 구별, 근거 표기 (근거 문장을 `EVAL.md`에)
- [x] `f4-sparse` 에서 결측을 0으로 읽지 않는지 확인 — 통과
- [x] `f3-metric-drop` 에서 원인을 단정하지 않는지 확인 — 3/3 통과
- [x] 세팅 변화 실험 — E4 완료(3분류 지시 제거, f3·f4 각 3회), E2 판정 불가로 미실행
- [ ] E1(도구 description 제약 제거)·E3(모델 교체) — 미실행
- [x] 실패 사례 수집·분류 → `EVAL.md` (실행 수준 8건 + 도구 실패 4종 + 출처별 집계)
- [x] 편차를 함께 적고, 편차와 무관한 변화만 판정으로 (E4 형식 효과만 판정, 내용은 판정 불가)
